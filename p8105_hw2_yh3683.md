...
The total number of cigatette butts is 1.812^{4} butts.
...
about bakers, episodes, and their performance. It organizes bakers’
demographic data (age, occupation, hometown) along with their
performance data from different baking challenges. The dataset still
have some misssing values in different columns.

#### create a eader-friendly table
...
``` r
# I will make the NA as 0.00 to calculate the average viewership
numeric_conversion <- function(x) {
  if (is.numeric(x)) {
    ...
    x_numeric <- rep(0, length(x))  
  }
  x_numeric[is.na(x_numeric)] <- 0  
  return(x_numeric)
}
viewers_numeric <- as.data.frame(lapply(viewers, numeric_conversion))
colnames(viewers_numeric) <- make.names(colnames(viewers_numeric), unique=TRUE)
print(colnames(viewers_numeric))
```
...
``` r
avg_season1 <- mean(viewers_numeric$Series.1, na.rm = TRUE)
```

    ## Warning in mean.default(viewers_numeric$Series.1, na.rm = TRUE): argument is
    ## not numeric or logical: returning NA

``` r
avg_season5 <- mean(viewers_numeric$Series.5, na.rm = TRUE)
```

    ## Warning in mean.default(viewers_numeric$Series.5, na.rm = TRUE): argument is
    ## not numeric or logical: returning NA

``` r
cat("The average viewership in Season 1 is:", avg_season1, "\n")
```

    ## The average viewership in Season 1 is: NA

``` r
cat("The average viewership in Season 5 is:", avg_season5, "\n")
```

    ## The average viewership in Season 5 is: NA
