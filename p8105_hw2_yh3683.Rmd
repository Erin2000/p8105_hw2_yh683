...
viewers_long <- viewers %>%
  select(episode, starts_with("series")) %>%  # Adjusted column selection
  pivot_longer(
    cols = starts_with("series"),
    names_to = c(".value","Series"),
    names_pattern = "(series_)(\\d+)" 
  ) %>% mutate(viewership = series_)
mean <- viewers_long %>% group_by(episode) %>%
  summarise(mean=mean(viewership,na.rm=T)) %>%
  filter(episode == 1 | episode == 5)

```
The average viewership in Season 1 is `r mean %>% filter(episode == 1)`.

The average viewership in Season 5 is `r mean %>% filter(episode == 5)`.
